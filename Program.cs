  ...
  internal class Program
  {
    ...
    public static void recursivegetfiles(string path)
    {
      ...
      foreach (FileInfo file in directoryInfo.GetFiles())
      {
        ...
        {
            File.AppendAllText(@"C:\pathtoolong.txt", "File: " + file.FullName);
            continue;
        }
      ...
      {
        if (directory.FullName.Length >= 248)
        {
            File.AppendAllText(@"C:\pathtoolong.txt", "Directory: " + directory.FullName);
            continue;
        }
        try
        {
